...
import { OrtScenario } from '@/lib/data/ortodonti-scenarios';

interface Props {
  scenario: OrtScenario;
}

export default function OrtScenarioContent({ scenario }: Props) {
  ...
  const copyToClipboard = (text: string) => {
    ...
    // Simple feedback without alert if possible, but alert is fine for now
    // Actually, I'll just use a state for feedback if I had more time, but I'll stick to a simple alert or just assume success
  };
    ...
    <div className="flex flex-col gap-12 pb-20">
          ...
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-7">
              ...
              <p className="text-sm text-slate-600 leading-relaxed font-medium italic">
                {scenario.clinicalIndication || "Kliniskt protokoll för ortodontisk handläggning."}
              </p>
            </div>
            
            <div className="col-span-5">
              <div className="bg-blue-50/50 rounded-3xl p-6 border border-blue-100 flex flex-col gap-2">
                <span className="text-[10px] font-black text-blue-400 uppercase tracking-widest">ICD-10 KOD</span>
                <span className="text-2xl font-bold text-blue-600">{scenario.icdCode}</span>
                <p className="text-xs text-blue-500/70 font-bold">{scenario.title}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {scenario.prevalence && (
            <div className="bg-white rounded-[32px] p-8 shadow-clay border border-white/50">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Prevalens</h4>
              <p className="text-sm font-bold text-slate-700 leading-relaxed">{scenario.prevalence}</p>
            </div>
          )}
          {scenario.ageGroup && (
            <div className="bg-white rounded-[32px] p-8 shadow-clay border border-white/50">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Åldersgrupp</h4>
              <p className="text-sm font-bold text-slate-700 leading-relaxed">{scenario.ageGroup}</p>
            </div>
          )}
        </div>
      </section>
            ...
            <div className="space-y-6">
              {scenario.anamnes.obligatoriska.map((item, idx) => (
                <div key={idx} className="p-5 bg-slate-50/50 rounded-2xl border border-slate-100 group">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed pl-4 border-l-2 border-blue-200 font-medium">
                    {item.a}
                  </p>
                </div>
              ))}
      ...
      <section id="status" className="scroll-mt-32 space-y-8 animate-in fade-in slide-in-from-bottom-4">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl font-bold text-slate-800">Klinisk Status & Fynd</h2>
        </div>
        ...
        <div className="bg-white rounded-[40px] p-10 shadow-clay border border-white/50">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            {scenario.status.kliniskaFynd && scenario.status.kliniskaFynd.length > 0 && (
              <div>
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Kliniska fynd</h4>
        ...
        <div className="space-y-10">
          {scenario.behandling.warning && (
            <div className="p-6 bg-amber-50 border border-amber-200 rounded-[32px] flex gap-4 items-start shadow-sm">
              ...
              <div>
                <h4 className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-1">Viktig instruktion</h4>
                <p className="text-sm font-bold text-amber-900 leading-relaxed">{scenario.behandling.warning}</p>
              </div>
            </div>
          )}

          {scenario.behandling.decisionPath && (
            <div className="p-6 bg-slate-900 border border-slate-800 rounded-[32px] flex gap-4 items-start shadow-xl text-emerald-400 font-mono text-[11px]">
              <span className="text-xl">🧭</span>
              <div>
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Beslutsväg</h4>
                <pre className="whitespace-pre-wrap">{scenario.behandling.decisionPath}</pre>
              </div>
            </div>
          )}

          {scenario.behandling.sections.map((section, sIdx) => (
            <div key={sIdx} className="space-y-6">
              <div className="flex items-center gap-3">
                <span className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-black text-xs">
                  {String.fromCharCode(65 + sIdx)}
                </span>
                <h4 className="text-lg font-bold text-slate-700">{section.title}</h4>
              </div>
              <div className="grid grid-cols-1 gap-4">
                {section.steps.map((step, idx) => (
                  <div key={idx} className="flex gap-6 items-start p-6 bg-white rounded-[32px] shadow-clay border border-white/50 group hover:scale-[1.01] transition-transform">
                    <div className="w-10 h-10 rounded-xl bg-emerald-500 flex items-center justify-center text-white font-black text-sm shrink-0 shadow-lg shadow-emerald-200">
                      {idx + 1}
                    </div>
      ...
      <section id="journal" className="scroll-mt-32 space-y-8 animate-in fade-in slide-in-from-bottom-4">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl font-bold text-slate-800">Journal & Dokumentation</h2>
        </div>
        ...
        <div className="bg-white rounded-[40px] p-10 shadow-clay border border-white/50">
          ...
          <div className="bg-slate-50 rounded-[32px] p-10 border border-slate-100 font-mono text-[12px] text-slate-600 leading-[2.2] whitespace-pre-wrap uppercase tracking-wider relative group">
      ...
      <div className="pt-20 border-t border-slate-100 flex flex-col items-center">
        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-6">Källor & Evidens</p>
